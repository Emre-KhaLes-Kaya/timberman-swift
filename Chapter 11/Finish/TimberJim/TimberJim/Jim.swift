...
class Jim: SKSpriteNode {
    ...
    func chopLeft() {
        ...
        self.run(Sound.sharedInstance.playSound(sound: .chop))
    }
    ...
    func animateEnd() {
        ...
        let texture = Textures.sharedInstance.textureWith(name: SpriteName.tombstone)
        
        self.texture = texture
        
        if self.position.x > kScreenCenter.x {
            self.xScale = -1
        }
    }
    ...
    func gameOver() {
        ...
        self.run(Sound.sharedInstance.playSound(sound: .gameOver))
